$(document).ready(function() {
	...
	// On click
	$('.clipped-box div a').on('click', function() {
		...
		if(clicked === false) {
			...
			$('.clipped-box>div:not(.content)').each(function() {
				...
				var color = $(this).css('backgroundImage').split('url(')[1].split(')')[0].split(', '),
				...
				z = setInterval(function() { 	
					...
					if(t > totalt) {
						...
						$('.clipped-box').css({'top' : '-2000px', 'transition' : 'none'});
	...
	r = setInterval(function() {
		...
		if(first === true) {
			...
			$('.clipped-box div').css({'opacity' : '1', 'transition' : '', 'background-image' : ''});
